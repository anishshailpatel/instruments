...
**Road by road** keeps your total customers fixed and spreads them across one to ten equal patches against a rival who keeps twenty in one. The cost of reaching a customer in a patch scales with the square root of area over customers (the travelling-salesman result), so your cost per customer against the rival's is the square root of (rival's customers ÷ your customers per patch): 0.45 at one patch, 1.0 at five, 1.41 at ten. The total never enters except through how thinly it is spread. The wording is deliberately generic (customers, patches, cost to reach) rather than vans and roads, so the lesson travels; the filename keeps the old name for URL stability.
